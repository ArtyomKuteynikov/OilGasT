...
from . import db
from .models import User, Codes, Invitations
...
def auth():
    ...
    ---
    get:
      summary: Проверка номера телефона существует ли в базе
      parameters:
          - in: query
            name: phone
            schema:
          ...
          content:
            application/json:
              schema:      # Request body contents
                  ...
                  example:   # Sample object
                    status: true
                    user: OPMrexrW7r9vdO0D$36e5126f0fe9fa66bd278c8c25dd3ad319179a0edf5de678f58a95a0042b343d
    ...
    try:
        ...
        if User.query.filter_by(phone=phone).first():
            ...
            if response.status_code == 200:
                ...
                client_info = response_data["data"]["client"]
                cards_info = response_data["data"]["cards"][0] if response_data["data"]["cards"] else {"balance": 0}
                name = client_info["lastName"]
                surname = client_info["firstName"]
                second_name = client_info["patronymic"]
                token = generate_password_hash(phone, method='sha256').replace('sha256$', '')
...
def sign_up():
    ...
    post:
      ...
      parameters:
          - in: query
            name: org
            schema:
              description: юрик или физик(1 если юрик, 0 если физик)
              type: integer
              example: 1
            description: юрик или физик(1 если юрик, 0 если физик)
          - in: query
              ...
              schema:      # Request body contents
                  ...
                  properties:
                    token:
                        type: string
                  example:   # Sample object
                    token: OPMrexrW7r9vdO0D$36e5126f0fe9fa66bd278c8c25dd3ad319179a0edf5de678f58a95a0042b343d
        '403':
    ...
    try:
        ...
        request_data = {
            "organizationId": organization_id,
            "departmentId": department_id,
            "requestId": "814ea82a-6387-4587-9cdc-791135586103",
        ...
        # Отправка POST-запроса
        response = requests.post(f"{base_url}/addClient", json=request_data, auth=auth, headers=headers)
        if response.status_code == 200:
            if org:
                new_user = User(phone=phone, name=name, surname=surname, org=1, org_name=org_name, status="active",
                                token=token, points=0, second_name=second_name,
                                registered=int(time.time()), photo='default_org.png')
                db.session.add(new_user)
                db.session.commit()
            else:
                new_user = User(phone=phone, name=name, surname=surname, org=1, second_name=second_name,
                                status="active", token=token, points=10, registered=int(time.time()))
                db.session.add(new_user)
                db.session.commit()
            return current_app.response_class(
                response=json.dumps(
...
@auth_api.route('/api/qr')
def qr():
    '''
    ---
    get:
      summary: Генерация QR кода юзера
      parameters:
        - in: query
          name: token
          schema:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            client = f'{cards_info["number"]}&{cards_info["id"]}'
        else:
            client = 'UNAUTHORIZED'
        pil_img = qrcode.make(client)
        img_io = BytesIO()
...
@auth_api.route('/api/loyalty-card')
def loyalty_card():
    '''
       ---
   get:
     summary: Бонусная карта
     parameters:
         - in: query
           name: token
           schema:
...
@auth_api.route('/api/notifications')
def notifications():
    '''
    ---
   get:
...
@auth_api.route('/api/notification')
def notification():
    '''
    ---
   get:
...
@auth_api.route('/api/invite')
def invite():
    '''
    ---
    get:
     summary: Пригласить друга/партнера
     parameters:
         - in: query
           name: token
           schema:
    ...
    try:
        ...
        if new_user:
            ...
            new_code = Invitations(code=code, inviter=user.id, org=user.org)
            db.session.add(new_code)
            db.session.commit()
            sender = Gate(SMS_LOGIN, SMS_PASSWORD)
            status = sender.send_message(phone,
                                         f'{user.name} {user.surname} пригласил вас присоединиться к приложению НефтеГазТ\nВведите этот код при авторизации{code}',
                                         'SMS DUCKOHT')
...
@auth_api.route('/api/review', methods=['POST'])
def review():
    '''
    ---
   post:
          ...
          application/json:
              schema:
                ...
                example:   # Sample object
                  ...
                  text: Классное приложение, разрабы молодцы!!!
     responses:
       '200':
         description: Результат
         content:
        ...
        response=json.dumps(
            {
                ...
                'status': 'Ваш отзыв успешно записан!'
            }
        ),
        status=200,
        mimetype='application/json'
